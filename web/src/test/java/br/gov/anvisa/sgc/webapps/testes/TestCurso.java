...
public class TestCurso extends TestBase{


	/** The Constant RESOURCE. */
	private static final String RESOURCE = "/api/curso";

	/** The Constant JSONPATH. */
	private static final String JSONPATH = "json/curso/";
	...
	public void sucessoConsultar() throws Exception { // NOPMD(Testes Unitários) by mario.melo
		getMockMvc().perform(get(RESOURCE)
				.param("page", "1")
				.param("count", "10")
				.param("descricao", "sistemas")
				).andDo( print() )
				.andExpect( status().isOk() )
				.andExpect( content().contentType(MediaType.APPLICATION_JSON) )
				.andExpect( jsonPath("$.resultado.dados[*].descricao", todosOsItens(containsString("SISTEMAS DA INFORMACAO") ) ));
	}

	@Test
	public void erroConsultar() throws Exception { // NOPMD(Testes Unitários) by mario.melo
		getMockMvc().perform( get(RESOURCE)
				.param("descricao", "dsadsadsadsads111")
				.param("page", "1")
				.param("count", "10")
				).andDo( print() )
				.andExpect( status().isOk() )
	...
	@Test
	@SqlPath(TestResourceSQL.SERVIDOR)
	public void altoCompleteNomeCurso() throws Exception { // NOPMD(Testes Unitários) by mario.melo
		getMockMvc().perform(get(RESOURCE + "/nome-curso")
				.param("value", "bio")
				).andDo( print() )
				.andExpect( status().isOk() )
				.andExpect( content().contentType(MediaType.APPLICATION_JSON) )
				.andExpect( jsonPath("$.resultado[*].descricao", todosOsItens(containsString("Biologia") ) ));
